...
def part1(training_data):
    ...
    with open(TWT_DEV_JSON, "r") as training_data:
        ...
        while tweet:
            ...
            tt += correct_total[1]
            tweet = training_data.readline()
    print("Dev: {}".format(tc/tt))
    # with open(TWT_TRAIN_JSON, "r") as training_data:
    #     tweet = training_data.readline()
    #     while tweet:
    #         correct_total = v.guess_sentence_tags(tweet)
    #         tc += correct_total[0]
    #         tt += correct_total[1]
    #         tweet = training_data.readline()
    # print("Train: {}".format(tc/tt))
    with open(TWT_TEST_JSON, "r") as training_data:
        tweet = training_data.readline()
        while tweet:
            correct_total = v.guess_sentence_tags(tweet)
            tc += correct_total[0]
